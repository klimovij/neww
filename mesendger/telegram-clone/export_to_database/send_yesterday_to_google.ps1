...
# Если скрипт запущен из планировщика задач (переменная окружения отсутствует), не ждем ввода
if ($env:USERNAME -and -not $env:USERPROFILE) {
    # Запущен из планировщика, не ждем ввода
    exit 0
}

# Иначе ждем ввода (если запущен вручную)
Read-Host "Нажмите Enter для выхода"
