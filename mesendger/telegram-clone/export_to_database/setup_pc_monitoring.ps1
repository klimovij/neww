...
if (Test-Path $startupScript) {
    ...
    Write-Host "✅ send_pc_startup.ps1 скопирован (отправляет включение + выключение из журнала)" -ForegroundColor Green
} else {
...
if (Test-Path $activityScript) {
    ...
    Write-Host "✅ send_activity.ps1 скопирован (скриншоты, сайты, приложения)" -ForegroundColor Green
} else {
...
# Удаляем старые задачи выключения (если они были установлены ранее)
Write-Host "`nУдаление старых задач выключения (если есть)..." -ForegroundColor Yellow
...
Write-Host "  schtasks /Query /TN Mesendger_PC_Activity_Monitor" -ForegroundColor Gray
...
Write-Host "     * Читает время ВЫКЛЮЧЕНИЯ из журнала Windows (EventID 6006)" -ForegroundColor DarkGray
Write-Host "     * Отправляет оба события с правильным временем" -ForegroundColor DarkGray
Write-Host "   - Mesendger_PC_Activity_Monitor - триггер: При входе пользователя" -ForegroundColor Gray
Write-Host "     * Отправляет скриншоты, данные о сайтах и приложениях" -ForegroundColor DarkGray
